...
def calc_perimeter(contour):
    # Периметр: сумма расстояний между последовательными точками контура
    perimeter = 0.0
...
def calc_area(contour):
    # Площадь можно вычислить с использованием формулы "Полигон Гаусса" (Shoelace formula)
    area = 0.0
    for i in range(len(contour)):
        x1, y1 = contour[i][0]
        x2, y2 = contour[(i + 1) % len(contour)][0]
...
def calc_roundness(perimeter, area):
    # Округлость: (4 * π * площадь) / (периметр^2), если периметр не равен 0
    if perimeter == 0:
        return 0
...
def classify_contours(contours, knn, image):
    ...
    for contour in contours:
        ...
        if predicted_label == 1:
            new_hue, shape_name = 0, "Круг"
        elif predicted_label == 2:
            new_hue, shape_name = 30, "Квадрат"
        elif predicted_label == 3:
            new_hue, shape_name = 60, "Шестиугольник"
        else:
            new_hue, shape_name = 120, "-"

        output_image = recolor(output_image, new_hue, contour)
        print(f"Контур: {shape_name} (Углы: {props[1]}, Округлость: {props[0]:.2f}, Периметр: {perimeter:.2f})")
    return output_image
...
output_path = "dataset_lab2out_knn"
...
for filename in os.listdir(dataset_path):
    if filename.endswith(('.jpg', '.jpeg', '.png')):
        ...
        # Изменение размера для соответствия целевой площади
        ...
        final_image = classify_contours(contours, knn, resized_image)
